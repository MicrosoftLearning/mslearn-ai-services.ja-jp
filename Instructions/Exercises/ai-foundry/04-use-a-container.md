...
> **注**:  
> Azure portal でコンテナーが "実行中" と表示されていても、必ずしも、リクエストを処理する準備ができているとは**限りません**。 `curl` を使用しようとすると、次のようなエラーが表示されます。
>
> ```text
...
> ```
>
> これは、コンテナーが正しいパラメーターまたは環境を使って起動されていないか、モデル ファイルまたはモデル ダウンロード用のインターネット アクセスがない可能性があります。

### 代替手段: Azure CLI を使用してコンテナーをデプロイする

次のコマンドを試すことができます (`rg-name`、`container-name`、`ApiKey`、`Billing` エンドポイントを実際の値に置き換えます)。
...
az container create \
  ...
  --os-type Linux
```
デプロイ後、エンドポイントをテストできます (<ACI_IP> を必要に応じて置き換えます)。
```bash
